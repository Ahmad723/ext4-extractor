...
if sys.hexversion < 0x02070000:
    ...
    print('sdat_to_img.py binary - version: %s\n' % __version__)

try:
    TRANSFER_LIST_FILE = str(sys.argv[1])
    NEW_DATA_FILE = str(sys.argv[2])
except IndexError:
    print('\nUsage: sdat_to_img.py <transfer_list> <system_new_file> [system_img]\n')
    print('    <transfer_list>: system.transfer.list file')
    print('    <system_new_file>: system.new.dat file')
    print('    [system_img]: output system image\n\n')
    print('Please visit google for more information.\n')
    try:
       input = raw_input
    except NameError: pass
    input('Press ENTER to exit...')
...
def main(argv):
    ...
    if version == 1:
        ...
        print('Android Marshmallow 6.0 / 6.0.1 detected!\n')
    elif version == 4:
